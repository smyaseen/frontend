...
import SignUpPage from '../useCases/performSignUp';
...
describe('SignUpPage', () => {
  it('Should renders name, email, and password inputs and submit button', () => {
    render(TestWrapper(<SignUpPage />));
  ...
  it('Should enables the submit button when name, email, and password are entered correctly', () => {
    render(TestWrapper(<SignUpPage />));
  ...
  it('Should disables the submit button when name is not full name', () => {
    render(TestWrapper(<SignUpPage />));

    fireEvent.change(screen.getByTestId('name'), { target: { value: 'John' } }); // Not a full name
    fireEvent.change(screen.getByTestId('email'), { target: { value: 'test@example.com' } });
    fireEvent.change(screen.getByTestId('password'), { target: { value: 'Egabcd@123' } });
  ...
  it('Should handles the whole journey correctly', () => {
    ...
    // Enter valid name, email, and invalid password
    fireEvent.change(screen.getByTestId('name'), { target: { value: 'John Doe' } });
    fireEvent.change(screen.getByTestId('email'), { target: { value: 'test@example.com' } });
